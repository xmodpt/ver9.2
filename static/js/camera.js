/**
 * Settings Tab Templates - WITH EXTERNAL CAMERA SETTINGS
 * Updated to use external MJPEG camera stream
 */
    ...
    general: (settings) => `
        <div class="settings-section">
            ...
            <div class="settings-form-group">
                <label>
                    <input type="checkbox" id="enableWebcam" ${settings.enableWebcam !== false ? 'checked' : ''}>
                    Enable Camera/Webcam
                </label>
                <div class="help-text">Enable camera streaming functionality</div>
            </div>
            
            <div class="settings-form-group">
                <label>
                    <input type="checkbox" id="autoStartCamera" ${settings.autoStartCamera !== false ? 'checked' : ''}>
                    Auto-Connect Camera
                </label>
                <div class="help-text">Automatically connect to camera when application loads</div>
            </div>
        ...
        <div class="settings-section">
            ...
            <div class="settings-section-description">
                Configure external MJPEG camera stream settings.
            </div>
        ...
        placeholder="Camera URL will be auto-detected">
                <div class="help-text">Complete URL to your MJPEG camera stream</div>
            </div>
            ...
            <div class="settings-form-group">
                <label>
                    <input type="checkbox" id="showCameraControls" ${settings.showCameraControls !== false ? 'checked' : ''}>
                    Show Camera Controls
                </label>
                <div class="help-text">Display camera control buttons overlay</div>
            </div>
...
// External camera stream test function
window.testCameraStream = async function() {
    const urlInput = document.getElementById('cameraStreamUrl');
    const streamUrl = urlInput ? urlInput.value : (window.cameraConfig?.streamUrl || '');
    
    if (!streamUrl.trim()) {
        if (typeof showAlert === 'function') {
            showAlert('Please enter a camera stream URL', 'warning');
        }
        return;
    }
    
    if (typeof showAlert === 'function') {
        showAlert('Testing camera stream...', 'info');
    }
    ...
    try {
        ...
        const testResult = await new Promise((resolve) => {
            ...
            img.src = streamUrl + (streamUrl.includes('?') ? '&' : '?') + 'test=' + Date.now();
        });
        
        if (testResult) {
            if (typeof showAlert === 'function') {
                showAlert('✅ Camera stream test successful!', 'success');
            }
            if (typeof addConsoleMessage === 'function') {
                addConsoleMessage(`Camera stream test passed: ${streamUrl}`, 'success');
            }
            console.log('📹 Camera stream test successful:', streamUrl);
        } else {
            if (typeof showAlert === 'function') {
                showAlert('❌ Camera stream test failed. Check URL and network connectivity.', 'error');
            }
            if (typeof addConsoleMessage === 'function') {
                addConsoleMessage(`Camera stream test failed: ${streamUrl}`, 'error');
            }
            console.error('📹 Camera stream test failed:', streamUrl);
        }
        
    } catch (error) {
        console.error('📹 Camera test error:', error);
        if (typeof showAlert === 'function') {
            showAlert(`Camera test error: ${error.message}`, 'error');
        }
        if (typeof addConsoleMessage === 'function') {
            addConsoleMessage(`Camera test error: ${error.message}`, 'error');
        }
    }
};
